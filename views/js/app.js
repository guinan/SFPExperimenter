...
function exportCSV(set) {
    ...
    toBeExported.push("URI,Timestamp,maxSearchDepth,maxSearchResults,maxPathLength,numRelevantNodes,Keywords,SFP");
    for(var i = 0; i < sfpList.length; i++) {
        if(sfpList[i][set])
            toBeExported.push(sfpList[i][0] + "," + sfpList[i][1] + "," + sfpList[i][2].replace(/;/g, ",") + "," + sfpList[i][4]);
    }

    // generate CSV
    var csvString = toBeExported.join("\r\n");
    var a = document.createElement("a");
    a.href = "data:attachment/csv," + encodeURIComponent(csvString);
    a.target = "_blank";
    a.download = "sfpexport.csv";
...
function canvasButtonOnClick(propTable, parentDiv, button) {
    return function() {
		...
		// TODO: if you want the amount of nodes, this is the place where you pull them from
